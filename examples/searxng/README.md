...
search engine, and mounts Exa's publication-specific index as the
`exa publications` engine. Exa remains behind SearXNG: Hound sees ordinary
search leads and never receives its API key.
...
Confirm both specialist engines and query Exa through Hound:

```bash
curl -fsS http://127.0.0.1:8080/config | jq \
  '.engines[] | select(.name == "federal register" or .name == "exa publications")'
