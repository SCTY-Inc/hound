"""Independent verifier for an isolated, unretained Slice 3B candidate."""
...
def _exact_observation(value: object) -> None:
    ...
    if value["schema_version"] != "houndd.slice3b.live-observations.v1":
        _fail("typed observations are invalid or have the wrong producer")
...
def _selection(name: object) -> tuple[str, ...]:
    if type(name) is not str or name not in SELECTIONS:
        _fail("evidence test selection is invalid")
...
def _run_id(value: object) -> str:
    if type(value) is not str or not value.startswith("slice3b-"):
...
def _collect(checkout: Path, python: str, selection: str) -> dict[str, object]:
    ...
    prefixes = tuple(f"{path}::" for path in _selection(selection))
...
def verify(
    ...
    *,
    ...
    required = {"schema_version", "selection", "run_id", "bb_thread_id", "source", "git", "interpreter", "pytest_environment", "argv", "source_files", "collection", "junit", "observations"}
    if type(manifest) is not dict or set(manifest) != required or manifest["schema_version"] != "houndd.slice3b.evidence.v5":
        _fail("manifest schema is not exact")
    if _selection(test_selection) != _selection(manifest["selection"]):
        _fail("evidence test selection binding is false")
    ...
    if manifest["argv"] != _run_argv(executable, test_selection):
    ...
    if (
        ...
        or set(collection) != {"schema_version", "selection", "argv", "environment", "exit_code", "node_ids"}
        or collection["schema_version"] != "houndd.slice3b.pytest-collection.v1"
        or _selection(collection["selection"]) != _selection(test_selection)
        or collection["argv"] != _collection_argv(executable, test_selection)
    ...
    with tempfile.TemporaryDirectory(prefix="hound-slice3b-verify-") as temporary:
        ...
        if _collect(checkout, executable, test_selection) != collection:
