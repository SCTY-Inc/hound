"""Generate an unretained Slice 3C1 candidate from an isolated source commit."""
...
def _run_id(value: str) -> str:
    if not value.startswith("slice3c1-"):
        raise ValueError("run ID must start with slice3c1-")
...
def generate(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="hound-slice3c1-source-") as temporary:
        ...
        work_evidence = checkout / "tests" / "evidence" / "slice3c1"
        work_evidence.mkdir(parents=True)
        collection_path = work_evidence / "slice3c1-collection.json"
        junit = work_evidence / "slice3c1-pytest.xml"
        observations = work_evidence / "observations.json"
        collection = _collect(checkout, str(interpreter["executable"]))
        collection_bytes = _canonical_json(collection)
        collection_path.write_bytes(collection_bytes)
        argv = _run_argv(str(interpreter["executable"]))
        result = subprocess.run(argv, cwd=checkout, env=_pytest_environment(), text=True, capture_output=True, check=False)
        if type(result.returncode) is not int or result.returncode != 0:
            raise RuntimeError(result.stdout + result.stderr)
        ...
        manifest = {
            "schema_version": "houndd.slice3c1.evidence.v1",
