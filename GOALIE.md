...
| B14 | pending | Descending journal.query (reverse cursor): houndd's cursor walks forward only, so any newest-first consumer view must walk from sequence 0 and cap (Workpad bounds at 1000 rows and reports truncation — beyond that it would show "newest of the oldest 1000"). Add a descending order option to journal.query with the same cursor integrity guarantees. | Newest-first page in one exchange; cursor tamper tests red. |
