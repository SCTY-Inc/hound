...
| B5 | complete (2026-08-04: `transcribe` is an adapter operation with no SOURCE — its bytes are the capture the daemon resolves for itself. Capture binding runs **before acceptance**: an ID must name an authorized completed `kind=media` event in the effective scope, whose record rehashes to its ID and whose blob verifies, or the request refuses with no reservation, attempt, record, event, or provider call (absent / out-of-scope / wrong-kind / near-miss ID are indistinguishable, the `ingest.url` lineage-refusal mapping). Record `houndd.transcript-record.v1` is **hashes and policy-safe provenance only** per VISION — capture binding, model/version/language, text hash + length, per-segment timing and text hashes — and **nothing is staged**, so the `houndd.phi-text-scan.v1` boundary stays exactly as VISION defines it (no `text/plain` extension). verify.py carries the transcript branch in the same change (B4 lesson): it re-resolves the named capture independently. 39 tests incl. crash matrix, negative bindings, degraded-without-key, injected-transport whisper suite; full suite 1913. **Open owner question:** consumers that need transcript *text* (radar-curation's transcript slice, M4) get nothing from this regime — VISION defers that to "a separately approved non-PHI representation", which does not exist. Not invented here; needs a D-row decision.) | Two-segment fixture: completed/partial/failed statuses stay explicit; no PHI in record. |
...
| B9 | complete (2026-08-05: resolved by B11's commit-path wiring — `commit_runtime.py::_complete_pair` calls `_refresh_projection` → `Projection.append` with `rebuild` fallback on every commit-finalize path incl. adapter and reconcile (commits `b0252be`, `3f52960`). Stale row — the "never touches Projection" claim predates that wiring, same pattern as E2/E3. Pinned by pre-existing `tests/test_b9_projection_refresh.py`, 5/5 green: live-commit index visibility without restart, replay idempotence, adapter path, refresh-failure isolation, crash/reconcile recovery.) | Commit-path projection refresh. | `tests/test_b9_projection_refresh.py` 5/5; `verify_store` green incl. projection on live-commit stores. |
...
| B14 | complete (2026-08-04, 0f94079: optional `order` field, additive — legacy cursors keep their exact digest domain; cross-order cursor reuse rejected via filter_hash domain separation; watermark pinning unchanged; 72 tests, mutation-tested. Surfaces' forward walks can now collapse to one descending page — follow-up with the surface-unification pass.) | Descending journal.query. | Cross-order/tamper/equivalence tests; mutation-tested. |
| B13 | complete (2026-08-04, acc98d8: optional bounded `options` on ingest.search, additive, retained in records; 42 tests incl. production-shape old-record compatibility. gc-web passthrough = B13b, in flight.) | ingest.search options. | Contract tests; old-record compatibility. |
...
| C2 | complete (2026-08-05: examples/consumer already carried the per-lane `<owner_id>-<policy_id>.consumer-state.json` + process-then-persist ordering via B10; proof strengthened with a genuine mid-drain SIGKILL+restart test — tests/test_consumer_replay.py 4/4: graceful restart, ungraceful SIGKILL, crash-before-persist, cursor-rejected resnapshot; docs/how-to-consume.md gained the B14 cross-order caveat (order folds into filter_hash — reset the cursor before switching order). Consumer audit: workpad intake-ledger is a stateless request-scoped descending walk (no persisted cursor by design — compliant trivially); gc-gtm-crm and signal-daily have no journal.query code yet, matching their baseline inventory rows. Operational note: houndd's RENAME_NOREPLACE socket bind relies on systemd RuntimeDirectory cleanup — outside systemd, a SIGKILL leaves a stale socket that blocks rebinding.) | Reference consumer pattern: per-lane cursor state file + replay discipline + how-to doc (Diátaxis how-to). | A sample consumer replays a cursor across a service restart without loss or duplicates. |
...
| D10 | decided 2026-08-05 (Ali) | Transcript representation: lanes transcribe OUTSIDE hound for non-evidence use. `houndd` keeps journaling media captures (hashes/provenance only, per VISION's PHI regime); transcript text is lane-local working material, never hound evidence. An approved-redaction pipeline is deferred until transcript text is needed AS evidence (re-entry = a new decision). Unblocks radar-curation/youtube (M4) design. |
| D11 | decided 2026-08-05 (Ali) | `full_cycle` semantics: at the migrated gate, `full_cycle` means "one real end-to-end run" (the Phase M replan's own wording), not necessarily a bb-scheduled cycle. benefits-radar's scheduled-cycle evidence remains valid as a superset. Pulse qualifies on run `pulse-2026-08-04`. |
| D12 | decided 2026-08-05 (Ali) | Residual provider keys: Exa/Firecrawl are stripped from bb's automation execution environment; `/home/deploy/.env` retains Exa/Firecrawl/Brave for non-GiveCare skill consumers (D7/D9 scope). gc-benefits' `benefit_engine` direct-Firecrawl pipeline is a named allowlisted exception (D1-deferred legacy ETL/intake tooling), tracked in `migration/domain-ownership-allowlist.v1.json`. The "provider credentials exist only inside houndd" acceptance sentence is scoped to migrated GiveCare lane acquisition. |
...
| ID | Status | Contract | Proof |
|---|---|---|---|
| E1 | complete (2026-08-04: migration/evidence/e1/ — five lanes re-proven credential-unset at HEAD against a keyless daemon, degraded-not-hang, all journaled; ownership scan clean in lane code) | Dynamic no-bypass matrix: every migrated consumer with credentials unset (HSP-18 full, HSP-01). Runs incrementally as lanes migrate. | Consumer matrix report; static scan with allowed exclusions only. |
| E2 | complete (built 2026-08-04 in 4f9ba3c — this row was stale until 2026-08-05: migration/stage_ledger.py + check_stage_ledger.py, 50 tests, CI-wired via the HSP-15 acceptance row; red cases cover skipped/reordered stages, migrated-without-approval, retired-without-gates. Residual: the chain has no external anchor — a self-consistent in-place re-derivation of historical entries passes verification; a git-anchored append-only check is tracked follow-up work.) | Stage-order checker + signed stage ledger (HSP-15). | Checker rejects skipped/reordered stages and pre-gate deletion. |
| E3 | complete (built 2026-08-04 in 4f9ba3c; row stale until the 2026-08-05 extension: DEFAULT_ROOTS were already whole-tree, but 285 .hound receipt false-positives + ~1900 pnpm-symlink walker failures made a clean red unproducible — fixed via .hound exclusion + directory-pruned walk, plus documentation-severity split, schema-validated reasoned allowlist (migration/domain-ownership-allowlist.v1.json, empty pending the owner exception-list decision), and line-level capability dump; 62 tests. Real-workspace proof: gc-benefits red with 13 live code violations (the D1-excluded benefit_engine direct-Firecrawl pipeline — NOT deleted by abffd89, contrary to the E1 lane evidence, which scanned only agents/ and must be corrected); full givecare scan at migration/evidence/e3/ownership-scan-givecare.json. Residual: HSP-19 acceptance row needs its two new evidence entries once acceptance.v1.json is free; per-lane scan_roots omit real driver code for wiki/intel/civic/pulse — widen with a CANONICAL_ROW_DIGEST recompute in the next inventory pass.) | Domain-ownership static checker + capability dump (HSP-19). | Ownership report: evidence mechanics only. |
| E4 | complete (2026-08-04, 653bf90 + d23f53f: docs/approval-seams.md design, migration/approvals checker (59 tests), and one REAL chain — benefits-radar gate receipt → hash-chained decision → stage-ledger approval_ref, HSP-22 walk green) | Approval seam records. | Tampered-decision-log and changed-hash fixtures fail the gate. |
| E5 | complete (2026-08-04, cb32e6f: tests/test_e5_fault_matrix.py + evidence inventory — 14 of 19 HSP-12 demands referenced to existing suites, 4 consolidated, backup-restore drill new; found and fixed the transcript projection boot-breaker in the same pass) | Consolidated fault matrix + backup-restore drill closing HSP-12 (transcript failure, outage abstention, exact-hash approval binding included). | Full matrix pytest log + restored-store verification. |
| E6 | complete (2026-08-04, e50c77d: migration/acceptance.v1.json claims all 22 HSP rows honestly — 15 complete/7 partial/0 open with per-row missing lists and live tripwires; check_acceptance enforces closed shape + one-to-one proof-artifact traceability; CI runs the acceptance command) | Full acceptance manifest: 22 ordered rows, one-to-one artifact traceability, CI runs the acceptance command (HSP-21). | Manifest checker + CI green. |
