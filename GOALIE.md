...
| B11 | pending | Incremental projection append: B9's per-commit refresh reuses the full `Projection.rebuild` (re-verifies every record hash, reads every staged blob) — O(journal) per commit. Fine at current size; becomes the dominant commit cost as the journal grows (M1's 249-record import batch is O(n²) blob reads). Add an incremental-append path on `Projection` with equality proven against full rebuild. | Property test: incremental result byte-identical to full rebuild across the commit matrix; commit latency bounded as journal grows. |
