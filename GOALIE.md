...
| B12 | pending | Commit-path journal chain re-validation is O(journal): `high_watermark`/`get`/`append` each re-read and re-verify the whole chain per commit — now the dominant per-commit cost (~66→153ms over 200 entries with projection refresh removed; journal is at 859). Found during B11. Bound it (e.g. verified-prefix cache with hash-anchored extension) without weakening chain integrity guarantees. | Operation-count proof that per-commit chain reads are O(1) amortized; tamper tests still red. |
