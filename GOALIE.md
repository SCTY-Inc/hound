...
| D10 | OPEN — needs Ali | Transcript representation: VISION's decided PHI regime (L1134-37) makes transcribe retain hashes/provenance only — consumers get NO transcript text from hound. The youtube-transcription and radar-curation transcript slices (M4) cannot be built until a "separately approved non-PHI representation" exists (options: an approved redaction/scan pipeline for transcript text as a scannable media type; or lanes transcribe outside hound for non-evidence use; or defer those slices). |
