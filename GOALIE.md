...
| M2 | in progress — **benefits-radar already CUT OVER** (gc-benefits abffd89, 2026-08-03: in-process houndd backend, shadow timer/script deleted, recorded E2E run; re-verified 2026-08-04 with a keyless ephemeral-daemon run classifying DEGRADED correctly; live write through the production daemon at 18:04Z). Stage-ledger `migrated` entry lands after its first full scheduled cycle (2026-08-04 06:35Z). Remaining: pulse-daily cutover (in flight, per the full-cutover replan — no parity ceremony). | Wave 2 cutover: pulse + benefits drivers acquire only via houndd, legacy deleted. | One real run per lane, credential-unset, correct bb terminal tokens. |
