...
def verify_store(root: str | Path, *, projection: bool = True) -> dict[str, Any]:
    ...
    if root_path.is_symlink() or not root_path.exists():
        ...
        return report
    try:
        records = RecordStore(root_path, create=False)
        journal = Journal(root_path, create=False)
        transactions = TransactionCoordinator(root_path, create=False)
    ...
    if projection:
        try:
            actual_rows = Projection(root_path).rows()
